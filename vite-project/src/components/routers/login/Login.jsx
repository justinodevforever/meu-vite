...
export default function Login() {
  ...
  async function hendleLogar(e) {
    ...
    setClick(true);
    const { data } = await chatflech.post("/logar", {
      email,
      password,
    });
    if (data.mensage !== "email ou senha Errada") {
      localStorage.setItem("refreshToken", data.refreshToken);
      localStorage.setItem(`token${data.User.dataValues.id}`, data.token);
      sessionStorage.setItem("user", data.User.dataValues.nome);
      sessionStorage.setItem("id", data.User.dataValues.id);
      dispatch(setId(data.User.dataValues.id));
